...
// Derivar sigla si carrera no tiene `sigla` en BD ("Ingeniería de Sistemas" -> "SIS")
const siglaFromNombreCarrera = (nombre = "") => {
  ...
  const last = parts[parts.length - 1]; // "sistemas"
  return last.slice(0,3).toUpperCase(); // "SIS"
};
...
const Docentes = () => {
  ...
  const [carreras, setCarreras] = useState([]);   // filtradas por facultad
  const [materias, setMaterias] = useState([]);   // filtradas por carrera
